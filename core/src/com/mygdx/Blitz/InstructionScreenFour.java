...
public class InstructionScreenFour extends ScreenAdapter {
    ...
    public InstructionScreenFour (Game game) {
        this.game = game;
    ...
    public void show() {
        ...
        dirTexture = new Texture(Gdx.files.internal("instru4.png"));
        ...
        back.addListener(new ActorGestureListener() {
            ...
            public void tap(InputEvent event, float x, float y, int count, int button) {
                super.tap(event, x, y, count, button);
                game.setScreen(new InstructionScreenThree(game));
                dispose();
            }
        });

        upgrade = new Texture(Gdx.files.internal("UpgradeL.png"));
        Image title = new Image(upgrade);
        title.setPosition(WORLD_WIDTH /2, (3*WORLD_HEIGHT) / 8,
                Align.center);
